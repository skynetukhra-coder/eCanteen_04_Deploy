...
router.post("/approve-user-recharge", async (req, res) => {
    try {
        ...
        if (!transaction_id) {
            return res.status(400).json({
                success: false,
                message: "Missing required transaction_id."
            });
        }

        // 1. Verify Admin (if admin_id and admin_password are provided)
        let adminName = "Admin";
        if (admin_id) {
            const [adminRows] = await db.query(
            ...
            if (adminRows.length > 0) {
                if (admin_password && adminRows[0].password !== admin_password) {
                    return res.status(403).json({
                        success: false,
        ...
        const detailsMsg = `Admin ${adminName} APPROVED UPI Wallet Recharge (TxID: #${transaction_id}) for Employee ${empInfo}. Amount: ₹${rechargeAmt.toFixed(2)}, Method: ${tx.payment_method || 'UPI'}, Transaction/UTR ID: ${tx.utr_number || 'N/A'}. New Balance: ₹${newBalance.toFixed(2)}.`;
        await db.query(
